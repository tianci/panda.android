...
public class FastBlur {
     ...
     */
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    public static void doBlur(Context context, Bitmap bitmap, View view, boolean isDownScale) {
        float scaleFactor = 1;
        float radius = 20;


        if (false && isDownScale) {
            scaleFactor = 8; //bitmap的尺寸缩小到原图的1/8
            radius = 2;
        }

        Log.d(TAG, "view.getMeasuredWidth() = " + view.getMeasuredWidth());
        Log.d(TAG, "view.getMeasuredHeight() = " + view.getMeasuredHeight());
